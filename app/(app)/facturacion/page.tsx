  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/80 overflow-y-auto py-6 px-4">
      <div className="w-full max-w-4xl space-y-3">
        ...
        {/* Toolbar */}
        <div className="flex items-center justify-between bg-[#141414] border border-[#2A2A2A] rounded-xl px-5 py-3">
          <div className="flex items-center gap-3">
            <FileText size={15} className="text-[#C9A84C]" />
            <span className="text-white font-semibold text-sm">Editor de factura</span>
            <span className="text-gray-600 text-xs">· todos los campos son editables</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => window.print()}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#2A2A2A] hover:bg-[#3A3A3A] text-white text-xs rounded-lg transition-colors">
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-white font-bold tracking-widest uppercase text-sm">Black Boats</span>
                ...
                <div className="space-y-0.5">
                  <input value={emisorNombre} onChange={e => setEmisorNombre(e.target.value)}
                    className="bg-transparent text-white font-semibold text-sm focus:outline-none border-b border-transparent hover:border-[#C9A84C]/20 focus:border-[#C9A84C]/50 w-full" />
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-end gap-3 text-xs">
                    ...
                    <input value={invoiceNum} onChange={e => setInvoiceNum(e.target.value)}
                      className="bg-transparent text-white font-mono font-bold text-right focus:outline-none border-b border-transparent hover:border-[#C9A84C]/20 focus:border-[#C9A84C]/50 w-52" />
                  </div>
                  <div className="flex items-center justify-end gap-3 text-xs">
                    <span className="text-gray-500">Fecha</span>
                    <input type="date" value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)}
                      className="bg-transparent text-white text-right focus:outline-none border-b border-transparent hover:border-[#C9A84C]/20 focus:border-[#C9A84C]/50 w-36" />
                  </div>
                  <div className="flex items-center justify-end gap-3 text-xs">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div>
                ...
                <div className="space-y-1">
                  ...
                  <input value={clientNif} onChange={e => setClientNif(e.target.value)}
                    ...
                    className="text-gray-600 text-xs focus:outline-none border-b border-transparent hover:border-gray-300 focus:border-gray-500 w-full bg-transparent" />
              ...
              {booking?.boat && (
                <div className="text-right">
                  ...
                  <p className="text-gray-500 text-xs">{booking.rental_type === 'with_captain' ? 'Con capitán' : 'Sin capitán'}</p>
                  {booking.start_date && (
            ...
            <div className="grid grid-cols-2 gap-8">
              ...
              <div>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
                  className="text-gray-600 text-xs focus:outline-none border-b border-transparent hover:border-gray-300 focus:border-gray-500 w-full bg-transparent resize-none" />
              </div>
            </div>
          </div>

          {/* Footer */}
          <div className="px-8 py-3 bg-[#0A0A0A] text-center">
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-1 bg-[#141414] border border-[#2A2A2A] rounded-xl p-1 w-fit">
        <button onClick={() => setTab('facturas')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'facturas' ? 'bg-[#C9A84C] text-black' : 'text-gray-400 hover:text-white'}`}>
          <FileText size={14} /> Emitir factura
        </button>
        <button onClick={() => setTab('pagos')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'pagos' ? 'bg-[#C9A84C] text-black' : 'text-gray-400 hover:text-white'}`}>
      ...
      {tab === 'facturas' && (
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#2A2A2A]">
            <p className="text-white font-semibold text-sm">Selecciona una reserva para generar su factura</p>
            <div className="relative">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Buscar..."
                className="pl-8 pr-4 py-2 bg-[#1E1E1E] border border-[#2A2A2A] rounded-lg text-xs text-white placeholder-gray-600 focus:outline-none focus:border-[#C9A84C]/50 w-44" />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#2A2A2A] bg-[#111]">
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Reserva</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Cliente</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Barco</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Fecha</th>
                  <th className="text-right px-5 py-3 text-gray-500 font-medium text-xs">Total</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Pago</th>
                    ...
                    <tr key={b.id} className="hover:bg-[#1A1A1A] transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-[#2A2A2A] flex items-center justify-center text-[#C9A84C] text-xs font-bold flex-shrink-0">
                            {clientName.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                          </div>
                          <span className="text-white text-sm">{clientName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <span className="text-white font-semibold">{Number(b.total_price).toLocaleString('es-ES')}€</span>
                      </td>
                      <td className="px-5 py-3.5">
      ...
      {tab === 'pagos' && (
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl overflow-hidden">
          ...
          {payments.length === 0 ? (
            <div className="text-center py-16 text-gray-500">No hay pagos registrados</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#2A2A2A] bg-[#111]">
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Reserva</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Cliente</th>
                  <th className="text-right px-5 py-3 text-gray-500 font-medium text-xs">Importe</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Método</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Fecha</th>
                  <th className="text-left px-5 py-3 text-gray-500 font-medium text-xs">Tipo</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1E1E1E]">
                {payments.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-[#1A1A1A] transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-white">{cname}</td>
                      <td className="px-5 py-3.5 text-right"><span className="text-white font-semibold">{Number(p.amount).toLocaleString('es-ES')}€</span></td>
