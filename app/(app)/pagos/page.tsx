    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl p-4">
          ...
          <p className="text-white text-2xl font-bold">{totalRecibido.toLocaleString('es-ES')}€</p>
        </div>
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl p-4">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-white text-2xl font-bold">{payments.filter(p => p.is_deposit).length}</p>
        </div>
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl p-4">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-white text-2xl font-bold">{completados}</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#2A2A2A]">
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium text-xs">Reserva</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium text-xs">Cliente</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium text-xs">Importe</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium text-xs">Método</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium text-xs">Fecha</th>
                <th className="text-left px-5 py-3.5 text-gray-500 font-medium text-xs">Tipo</th>
              </tr>
            </thead>
              ...
              {payments.map(p => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-[#1A1A1A] transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-white">{clientName}</td>
                    <td className="px-5 py-3.5">
                      <span className="text-white font-semibold">{Number(p.amount).toLocaleString('es-ES')}€</span>
                    </td>
                    <td className="px-5 py-3.5">
